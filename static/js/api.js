...
// Auth API — cadastro com username (email/senha opcionais); login sem senha; device_id para HWID/trusted device
const AuthAPI = {
    async register(username, email, password, deviceId) {
        const data = await apiRequest('/auth/register', {
            method: 'POST',
            body: { username: username || undefined, email: email || undefined, password: password || undefined, device_id: deviceId || undefined }
        });
    ...
    async login(username, password, deviceId) {
        const data = await apiRequest('/auth/login', {
            method: 'POST',
            body: { username: username || undefined, password: password || undefined, device_id: deviceId || undefined }
        });
        TokenManager.setTokens(data.access_token, data.refresh_token);
        return data;
    },
