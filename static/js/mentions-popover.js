...
(function (global) {
    ...
    function getCaretCoordinates(element, position) {
        ...
        div.style.cssText = 'position:absolute;white-space:pre-wrap;word-wrap:break-word;top:0;left:0;visibility:hidden;font:' + style.font + ';padding:' + style.padding + ';border:' + style.border + ';line-height:' + style.lineHeight + ';width:' + element.offsetWidth + 'px;';
    ...
    function showPopover(input, list, selectedIndex, onSelect) {
        ...
        list.slice(0, MAX_ITEMS).forEach((member, i) => {
            ...
            const avatar = member.avatar_url || member.avatar ? `<img src="${member.avatar_url || member.avatar}" alt="">` : '';
    ...
    function getQueryAndStartPos(text, caretPos) {
        ...
        const query = text.substring(start + 1, caretPos).trim().toLowerCase();
        ...
        init(app) {
            ...
            function getMembers() {
                ...
                if (ch.recipients && ch.recipients.length) {
                    return ch.recipients.map(r => ({ id: r.id, username: r.username || r.display_name || 'User', avatar_url: r.avatar_url || r.avatar }));
                }
