...
(function () {
    ...
    function tempId() {
        return (typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID() : 'pending-' + Date.now() + '-' + Math.random().toString(36).slice(2);
    }
        ...
        useEffect(function () {
            ...
            function handler(msg) {
                ...
                setMessages(function (prev) {
                    ...
                    var fromSelf = normalized.author_id && currentUserRef.current && String(normalized.author_id) === String(currentUserRef.current.id);
                    if (fromSelf) {
        ...
        useEffect(function () {
            window.LibertyChatSendMessage = function (content) {
                ...
                return API.Message.create(roomOrId, content, false, [], tempIdVal)
                    ...
                    .catch(function (err) {
                        setMessages(function (prev) {
                            var next = prev.filter(function (m) { return (m.id || m.message_id) !== tempIdVal; });
                            appInst.setMessagesFromList(next);
                            return next;
                        });
                        throw err;
