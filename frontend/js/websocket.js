    ...
    handleMessage(message) {
        ...
        switch (op) {
            case 'hello':
                ...
                console.log('[Gateway] Server version:', (d && d.server_version) || '1.0');
                var tok = typeof API !== 'undefined' && API.Token ? API.Token.getAccessToken() : (localStorage.getItem('access_token') || '');
                if (tok) this.send('authenticate', { token: tok });
                break;
            case 'authenticated':
                this.authenticated = true;
                this.sessionId = (d && d.session_id) || null;
                console.log('[Gateway] Authenticated as', (d && d.user && d.user.username) || 'user');
                this.emit('ready', d || {});
                this._resolveConnect(d || {});
                break;
            case 'auth_failed':
                console.error('[Gateway] Auth failed:', (d && d.reason) || 'unknown');
                this.emit('auth_failed', d || {});
                if (typeof this._connectReject !== 'function') break;
                var rejectFnAuth = this._connectReject;
                this._connectResolve = null;
                this._connectReject = null;
                try { rejectFnAuth(new Error((d && d.reason) || 'Auth failed')); } catch (_) {}
                break;
