...
async function ensureMessageInDb(msg, chatId) {
  ...
  try {
    ...
    const authorId = msg.author_id || msg.author_id === 0 ? String(msg.author_id) : null;
    const createdAt = msg.created_at || msg.timestamp || new Date();
    
    await db.query(
      `INSERT INTO messages (id, chat_id, user_id, content, created_at)
       VALUES ($1::uuid, $2::uuid, $3::uuid, $4, $5)
       ON CONFLICT (id) DO NOTHING`,
      [msgId, chatId, authorId, content, createdAt]
    );
  } catch (err) {
    logger.error('ensureMessageInDb', err);
  }
}
...
async function loadMessagesFromDb(chatId, limit = 300) {
  ...
  try {
    ...
    logger.error('loadMessagesFromDb', err);
    return [];
  }
}
...
CREATE INDEX IF NOT EXISTS idx_tasks_status ON tasks(assignee_id, status);
`;
...
async function start() {
  ...
  app.locals.emitMessage = function (message) {
    ...
    // Emitir para AMBOS: chatId e channelId para garantir que todos recebam
    if (chatId) {
      io.to(String(chatId)).emit('message', payload);
      const room = io.sockets.adapter.rooms.get(String(chatId));
      console.log('[MSG] Sockets na sala chatId', chatId, ':', room ? room.size : 0);
    }
    if (channelId && String(channelId) !== String(chatId)) {
      io.to(String(channelId)).emit('message', payload);
      const room = io.sockets.adapter.rooms.get(String(channelId));
      console.log('[MSG] Sockets na sala channelId', channelId, ':', room ? room.size : 0);
    }
  };

  io.use((socket, next) => {
    // 1. Tenta pegar o userId do handshake (vindo do cliente) - PRIORIDADE MÁXIMA
    const userId = socket.handshake.auth?.userId || socket.handshake.query?.userId;
    if (userId) {
      socket.userId = String(userId);
      console.log('[Socket.IO] userId definido via handshake:', socket.userId);
      return next();
    }
    
    // 2. Fallback: tenta extrair do JWT token
    const token =
    ...
    if (payload?.sub) {
      ...
      console.log('[Socket.IO] userId definido via JWT:', socket.userId);
      return next();
    }
    
    // 3. Sem autenticação - permite conectar mas sem userId
    console.warn('[Socket.IO] Socket conectado SEM userId - handshake:', {
  ...
  io.on('connection', socket => {
    // Log básico para qualquer conexão
    console.log('🔌 Nova conexão detectada:', socket.id);
    
    // Middleware já definiu socket.userId, agora apenas criar a sala
    if (socket.userId) {
      socket.join(socket.userId);
      console.log(`✅ [Socket] Usuário ${socket.userId} autenticado e sala criada. Socket ID: ${socket.id}`);
      
      if (!io._libertyUserSockets) io._libertyUserSockets = new Map();
      let set = io._libertyUserSockets.get(socket.userId);
      if (!set) {
        set = new Set();
        io._libertyUserSockets.set(socket.userId, set);
      }
      set.add(socket.id);
    } else {
      console.warn('⚠️ [Socket] Conexão sem userId detectada. Sinais de chamada podem falhar.');
    }
    
    // Listener de emergência para verificar identidade
    socket.on('verify_identity', (data) => {
      console.log('📡 [VERIFY] Recebido verify_identity:', data);
      if (data && data.userId) {
        socket.userId = String(data.userId);
        socket.join(socket.userId);
        console.log(`✅ [VERIFICADO] Sala forçada para: ${socket.userId}`);
    ...
    // Listener de autenticação complementar
    socket.on('authenticate', (data) => {
      console.log('📡 [AUTH] Recebido authenticate:', data);
      if (data && data.userId) {
        socket.userId = String(data.userId);
        socket.join(socket.userId);
        console.log(`✅ [SQUARE CLOUD] Usuário autenticado: ${socket.userId} | Socket: ${socket.id}`);
        ...
        socket.emit('authenticated', { userId: socket.userId, socketId: socket.id });
      }
    });
    
    if (!io._voiceCalls) io._voiceCalls = new Map();
    if (!io._voiceCallsByUser) io._voiceCallsByUser = new Map();
    socket.on('subscribe', payload => {
      const chatId = payload && (payload.chat_id || payload.chatId);
      if (chatId) {
        socket.join(String(chatId));
        console.log('[SUBSCRIBE] Socket', socket.id, 'entrou na sala:', chatId);
        const room = io.sockets.adapter.rooms.get(String(chatId));
        console.log('[SUBSCRIBE] Total na sala', chatId, ':', room ? room.size : 0);
      }
    });
    ...
    // Teste de Eco - verificar se socket está vivo
    socket.on('test_ping', () => {
      console.log(`[TEST] Ping recebido de ${socket.userId}, respondendo pong`);
      socket.emit('test_pong', { msg: 'Socket Vivo!', userId: socket.userId });
    });

    // WebRTC Signaling - Encaminhamento com verificação de sala
    socket.on('call:request', data => {
      const targetId = String(data.to);
      const fromId = String(socket.userId);
      
      console.log(`📞 [CALL] Chamada: de ${fromId} para ${targetId}`);
      console.log(`📞 [CALL] _libertyUserSockets:`, io._libertyUserSockets ? Array.from(io._libertyUserSockets.keys()) : 'VAZIO');
      
      if (!data || !data.to) {
        console.log('[CALL] call:request sem destino valido');
        socket.emit('call:error', { message: 'Destino inválido.' });
        return;
      }
      
      const callId = data.callId || crypto.randomUUID();
      
      // VERIFICAR SE O DESTINO EXISTE via _libertyUserSockets
      const targetSockets = io._libertyUserSockets && io._libertyUserSockets.get(targetId);
      console.log(`🔍 [CALL] Sockets de ${targetId}:`, targetSockets ? Array.from(targetSockets) : 'NENHUM');
      ...
      if (targetSockets && targetSockets.size > 0) {
        ...
        // Registrar a chamada
        if (!io._voiceCalls) io._voiceCalls = new Map();
        io._voiceCalls.set(callId, { callId, callerId: fromId, calleeId: targetId });
        
        console.log(`📡 [SQUARE CLOUD] Sinal ENTREGUE com sucesso a ${targetId}`);
        
        // Confirmar para o caller
        socket.emit('call:sent', { callId, to: targetId });
      } else {
        console.log(`❌ [SQUARE CLOUD] Falha: Destinatário ${targetId} está OFFLINE no socket.`);
        socket.emit('call:error', { message: 'Utilizador offline ou sem socket ativo.' });
      }
    });

    socket.on('call:reject', data => {
      try {
        const fromUserId = socket.userId ? String(socket.userId) : null;
        const callId = data && data.callId ? String(data.callId) : null;
        if (!fromUserId || !callId) return;
        const c = io._voiceCalls.get(callId);
        if (!c) return;
        const callerSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.callerId));
        if (callerSockets) {
          for (const sid of callerSockets) io.to(sid).emit('call:rejected', { callId, from: fromUserId });
        }
        io._voiceCalls.delete(callId);
      } catch (e) {}
    });
    ...
    socket.on('call:end', data => {
      try {
        ...
        if (!c) return;
        const callerSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.callerId));
        const calleeSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.calleeId));
        if (callerSockets) {
          for (const sid of callerSockets) io.to(sid).emit('call:ended', { callId, from: fromUserId });
        }
        if (calleeSockets) {
          for (const sid of calleeSockets) io.to(sid).emit('call:ended', { callId, from: fromUserId });
        }
        io._voiceCalls.delete(callId);
      } catch (e) {}
    });
    ...
    socket.on('call:busy', data => {
      console.log('[CALL] call:busy recebido de:', socket.userId, 'para:', data?.to);
      if (!data || !data.to) return;
      socket.to(String(data.to)).emit('call:busy', {
        from: socket.userId,
        ...data
      });
    });
    ...
    socket.on('call:start', payload => {
      console.log('Recebi pedido de call de:', socket.userId);
      try {
        const fromUserId = socket.userId ? String(socket.userId) : null;
        const toUserId = payload && payload.to ? String(payload.to) : null;
        const callId = payload && payload.callId ? String(payload.callId) : crypto.randomUUID();
        if (!fromUserId || !toUserId) return;
        const calleeSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(toUserId));
        if (!calleeSockets || calleeSockets.size === 0) {
          socket.emit('call:error', { callId, message: 'Usuário offline.' });
          return;
        }
        io._voiceCalls.set(callId, { callId, callerId: fromUserId, calleeId: String(toUserId) });
        let s1 = io._voiceCallsByUser.get(fromUserId);
        if (!s1) { s1 = new Set(); io._voiceCallsByUser.set(fromUserId, s1); }
        s1.add(callId);
        let s2 = io._voiceCallsByUser.get(String(toUserId));
        if (!s2) { s2 = new Set(); io._voiceCallsByUser.set(String(toUserId), s2); }
        s2.add(callId);
        for (const sid of calleeSockets) io.to(sid).emit('call:incoming', { callId, from: fromUserId });
      } catch (e) {}
    });

    socket.on('call:accept', payload => {
      console.log('[CALL] call:accept recebido:', { userId: socket.userId, payload });
      try {
        const fromUserId = socket.userId ? String(socket.userId) : null;
        const callId = payload && payload.callId ? String(payload.callId) : null;
        console.log('[CALL] Dados:', { fromUserId, callId });
        if (!fromUserId || !callId) { console.log('[CALL] Faltando dados'); return; }
        const c = io._voiceCalls.get(callId);
        console.log('[CALL] Chamada encontrada:', c);
        if (!c) { console.log('[CALL] Chamada não existe'); return; }
        if (String(c.calleeId) !== fromUserId) { console.log('[CALL] Callee não bate:', { calleeId: c.calleeId, fromUserId }); return; }
        const callerSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.callerId));
        console.log('[CALL] Caller sockets:', callerSockets ? callerSockets.size : 0);
        if (!callerSockets || callerSockets.size === 0) { console.log('[CALL] Caller offline'); return; }
        for (const sid of callerSockets) {
          console.log('[CALL] Enviando call:accepted para:', sid);
          io.to(sid).emit('call:accepted', { callId, to: String(c.calleeId), from: String(c.callerId) });
        }
        const calleeSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.calleeId));
        if (calleeSockets) for (const sid of calleeSockets) io.to(sid).emit('call:accepted', { callId, to: String(c.callerId), from: String(c.calleeId) });
      } catch (e) { console.error('[CALL] Erro:', e); }
    });
    ...
    socket.on('call:reject', payload => {
      try {
        ...
        const c = io._voiceCalls.get(callId);
        if (!c) return;
        const isCaller = String(c.callerId) === fromUserId;
        const isCallee = String(c.calleeId) === fromUserId;
        if (!isCaller && !isCallee) return;
        const callerSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.callerId));
        const calleeSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.calleeId));
        if (callerSockets) for (const sid of callerSockets) io.to(sid).emit('call:ended', { callId, from: fromUserId });
        if (calleeSockets) for (const sid of calleeSockets) io.to(sid).emit('call:ended', { callId, from: fromUserId });
        io._voiceCalls.delete(callId);
      } catch (e) {}
    });

    socket.on('webrtc_signal', payload => {
      console.log('[WEBRTC] Sinal recebido:', { from: socket.userId, kind: payload?.kind, to: payload?.to });
      try {
        const fromUserId = socket.userId ? String(socket.userId) : null;
        const callId = payload && payload.callId ? String(payload.callId) : null;
        const to = payload && payload.to ? String(payload.to) : null;
        const kind = payload && payload.kind ? String(payload.kind) : null;
        const data = payload && payload.data ? payload.data : null;
        if (!fromUserId || !callId || !to || !kind || !data) {
          console.log('[WEBRTC] Dados faltando');
          return;
        }
        const toSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(to));
        console.log('[WEBRTC] Enviando para:', to, 'sockets:', toSockets ? toSockets.size : 0);
        if (!toSockets || toSockets.size === 0) return;
        for (const sid of toSockets) {
          console.log('[WEBRTC] Enviando para socket:', sid);
          io.to(sid).emit('webrtc_signal', { callId, from: fromUserId, kind, data });
        }
      } catch (e) { console.error('[WEBRTC] Erro:', e); }
    });
    ...
    socket.on('request-voice-call', payload => {
      try {
        const fromUserId = socket.userId ? String(socket.userId) : null;
        const toUserId = payload && payload.to ? String(payload.to) : null;
        const offer = payload && payload.offer ? payload.offer : null;
        const callId = payload && payload.callId ? String(payload.callId) : crypto.randomUUID();
        if (!fromUserId || !toUserId || !offer) return;

        const calleeSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(toUserId));
        if (!calleeSockets || calleeSockets.size === 0) {
          socket.emit('voice:call-error', { callId, message: 'Usuário offline.' });
          return;
        }
        ...
        if (!s1) {
          s1 = new Set();
          io._voiceCallsByUser.set(fromUserId, s1);
        }
        s1.add(callId);
        ...
        if (!s2) {
          s2 = new Set();
          io._voiceCallsByUser.set(String(toUserId), s2);
        }
        s2.add(callId);

        for (const sid of calleeSockets) {
          io.to(sid).emit('voice:call-incoming', {
            callId,
            fromId: fromUserId,
            fromInfo: payload && payload.fromInfo ? payload.fromInfo : null,
            offer,
          });
        }
      } catch (err) {
    ...
    socket.on('voice:call-answer', payload => {
      try {
        ...
        if (!fromUserId || !callId || !answer) return;

        const c = io._voiceCalls.get(callId);
        if (!c) return;
        if (String(c.calleeId) !== fromUserId) return;

        const callerSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.callerId));
        if (!callerSockets || callerSockets.size === 0) return;

        for (const sid of callerSockets) {
          io.to(sid).emit('voice:call-answered', {
            callId,
            fromId: fromUserId,
            fromInfo: payload && payload.fromInfo ? payload.fromInfo : null,
            answer,
    ...
    socket.on('voice:call-ice', payload => {
      try {
        const fromUserId = socket.userId ? String(socket.userId) : null;
        const callId = payload && payload.callId ? String(payload.callId) : null;
        const candidate = payload && payload.candidate ? payload.candidate : null;
        if (!fromUserId || !callId || !candidate) return;
        ...
        const toUserId = isCaller ? String(c.calleeId) : String(c.callerId);
        const toSockets = io._libertyUserSockets && io._libertyUserSockets.get(toUserId);
        if (!toSockets || toSockets.size === 0) return;

        for (const sid of toSockets) {
          io.to(sid).emit('voice:call-ice', { callId, fromId: fromUserId, candidate });
        }
      } catch (err) {
        console.error(err);
      }
    ...
    socket.on('voice:speaking', payload => {
      try {
        const fromUserId = socket.userId ? String(socket.userId) : null;
        const callId = payload && payload.callId ? String(payload.callId) : null;
        const isSpeaking = !!(payload && payload.isSpeaking);
        if (!fromUserId || !callId) return;
        ...
        for (const sid of toSockets) {
          io.to(sid).emit('voice:speaking', { callId, fromId: fromUserId, isSpeaking });
        }
      } catch (err) {
        console.error(err);
      }
    ...
    socket.on('voice:call-end', payload => {
      try {
        ...
        const calleeSockets = io._libertyUserSockets && io._libertyUserSockets.get(String(c.calleeId));
        if (callerSockets) {
          for (const sid of callerSockets) io.to(sid).emit('voice:call-ended', { callId, fromId: fromUserId });
        }
        if (calleeSockets) {
          for (const sid of calleeSockets) io.to(sid).emit('voice:call-ended', { callId, fromId: fromUserId });
        }

        io._voiceCalls.delete(callId);

        const ids = [String(c.callerId), String(c.calleeId)];
        for (let i = 0; i < ids.length; i += 1) {
          const uid = ids[i];
          const set = io._voiceCallsByUser && io._voiceCallsByUser.get(uid);
          if (set) {
            set.delete(callId);
            if (set.size === 0) io._voiceCallsByUser.delete(uid);
          }
        }
      } catch (err) {
        console.error(err);
    ...
    socket.on('disconnect', () => {
      ...
      if (set.size === 0) {
        io._libertyUserSockets.delete(uid);
        const callIds = io._voiceCallsByUser && io._voiceCallsByUser.get(uid);
        if (callIds && callIds.size > 0) {
          for (const callId of callIds) {
            const c = io._voiceCalls.get(callId);
            if (!c) continue;
            const otherId = String(c.callerId) === uid ? String(c.calleeId) : String(c.callerId);
            const otherSockets = io._libertyUserSockets && io._libertyUserSockets.get(otherId);
            if (otherSockets && otherSockets.size > 0) {
              for (const sid of otherSockets) io.to(sid).emit('voice:call-ended', { callId, fromId: uid });
            }

            io._voiceCalls.delete(callId);
            for (const uid2 of [String(c.callerId), String(c.calleeId)]) {
              const set2 = io._voiceCallsByUser && io._voiceCallsByUser.get(uid2);
              if (set2) {
                set2.delete(callId);
                if (set2.size === 0) io._voiceCallsByUser.delete(uid2);
              }
            }
          }
        }
  ...
  app.post('/api/messages', auth.requireAuth, validateBody(schemas.messageContent), async (req, res) => {
    ...
    try {
      ...
      ensureMessageInDb(saved, chatId); // Persistir no banco
  ...
  app.get('/api/messages', auth.requireAuth, async (_req, res) => {
    try {
      ...
      let list = messageCache.list(chatId);
      
      // Se cache vazio, carregar do banco
      if (list.length === 0 && db.isConnected()) {
        list = await loadMessagesFromDb(chatId);
        // Repopular o cache
        list.forEach(m => messageCache.push(chatId, m));
      }
  ...
  app.post('/api/v1/calls', auth.requireAuth, async (req, res) => {
    ...
    if (!fromUserId) return res.status(401).json({ message: 'Não autorizado' });
    ...
    const targetUserId =
      body.callee_id != null
        ...
        : body.calleeId != null
          ? String(body.calleeId)
          : body.caller_id != null && body.callee_id == null && body.to == null && body.target_user_id == null && body.to_user_id == null
            ? null
            :
    ...
    const kindRaw = body.kind != null ? body.kind : body.type != null ? body.type : null;
    const kind = typeof kindRaw === 'string' ? kindRaw.trim().toLowerCase() : '';

    let msgType =
      kind === 'offer' ? 'webrtc_offer'
        : kind === 'answer' ? 'webrtc_answer'
          : kind === 'ice' ? 'webrtc_ice'
            : kind === 'webrtc_offer' ? 'webrtc_offer'
              : kind === 'webrtc_answer' ? 'webrtc_answer'
                : kind === 'webrtc_ice' ? 'webrtc_ice'
                  : null;

    if (!msgType) {
      if (body.answer != null || (body.payload && body.payload.type === 'answer')) msgType = 'webrtc_answer';
      else if (body.candidate != null || body.ice_candidate != null || body.iceCandidate != null || (body.payload && body.payload.candidate != null)) msgType = 'webrtc_ice';
      else if (body.offer != null || body.sdp != null || body.answer_sdp != null || (body.payload && body.payload.type === 'offer')) msgType = 'webrtc_offer';
    }

    if (!targetUserId || !msgType) {
      if (targetUserId && !msgType) {
        const callId =
          body.call_id != null ? String(body.call_id)
            : body.callId != null ? String(body.callId)
              : body.chat_id != null ? String(body.chat_id)
                : body.chatId != null ? String(body.chatId)
                  : crypto.randomUUID();
        return res.status(200).json({ ok: true, call_id: callId });
      }
      return res.status(400).json({
        message: 'Parâmetros inválidos (target_user_id/msgType)',
        received: {
          callee_id: body.callee_id != null ? body.callee_id : null,
          calleeId: body.calleeId != null ? body.calleeId : null,
          chat_id: body.chat_id != null ? body.chat_id : null,
          chatId: body.chatId != null ? body.chatId : null,
          target_user_id: body.target_user_id != null ? body.target_user_id : null,
          targetUserId: body.targetUserId != null ? body.targetUserId : null,
          to_user_id: body.to_user_id != null ? body.to_user_id : null,
          toUserId: body.toUserId != null ? body.toUserId : null,
          to: body.to != null ? body.to : null,
          kind,
          hasPayload: body.payload != null,
          hasOffer: body.offer != null,
          hasAnswer: body.answer != null,
          hasCandidate: body.candidate != null,
          hasSdp: body.sdp != null,
        },
      });
    }

    let payload = null;
    if (body.payload != null) payload = body.payload;
    else if (msgType === 'webrtc_ice')
      payload = body.candidate != null
        ? body.candidate
        : body.ice_candidate != null
          ? body.ice_candidate
          : body.iceCandidate != null
            ? body.iceCandidate
            : null;
    else if (msgType === 'webrtc_answer')
      payload = body.answer != null
        ? body.answer
        : body.sdp != null
          ? { type: 'answer', sdp: body.sdp }
          : body.answer_sdp != null
            ? { type: 'answer', sdp: body.answer_sdp }
            : null;
    else if (msgType === 'webrtc_offer')
      payload = body.offer != null
        ? body.offer
        : body.sdp != null
          ? { type: 'offer', sdp: body.sdp }
          : body.offer_sdp != null
            ? { type: 'offer', sdp: body.offer_sdp }
            : null;

    if (!payload) {
      return res.status(400).json({
        message: 'Parâmetros inválidos (payload)',
        received: { msgType, hasPayload: body.payload != null, hasOffer: body.offer != null, hasAnswer: body.answer != null, hasCandidate: body.candidate != null, hasSdp: body.sdp != null },
      });
    }

    if ((msgType === 'webrtc_offer' || msgType === 'webrtc_answer') && typeof payload === 'string') {
      payload = { type: msgType === 'webrtc_answer' ? 'answer' : 'offer', sdp: payload };
    }

    if (payload && typeof payload === 'object') {
      if (msgType === 'webrtc_offer' && payload.offer) payload = payload.offer;
      else if (msgType === 'webrtc_answer' && payload.answer) payload = payload.answer;
      else if (msgType === 'webrtc_ice' && payload.candidate) payload = payload.candidate;
    }

    _wsSendToUser(targetUserId, {
      type: msgType,
      from_user_id: String(fromUserId),
      call_id: body.call_id != null ? String(body.call_id) : null,
      payload
    });
  ...
  // ── Tasks ─────────────────────────────────────────────────────
  // GET /api/v1/users/:userId/tasks — listar tarefas de um usuário
  app.get('/api/v1/users/:userId/tasks', auth.requireAuth, requireUuidParams(['userId']), async (req, res) => {
    if (!db.isConfigured() || !db.isConnected()) return res.status(503).json({ message: 'Banco indisponível' });
    const userId = req.userId;
    const targetId = req.params.userId;
    const status = req.query.status;
    try {
      let sql = `SELECT t.*, c.username as creator_username, a.username as assignee_username
                 FROM tasks t
                 JOIN users c ON c.id = t.creator_id
                 JOIN users a ON a.id = t.assignee_id
                 WHERE t.assignee_id = $1::uuid OR t.creator_id = $1::uuid`;
      ...
      logger.error('GET /api/v1/users/:userId/tasks', err);
      return res.status(500).json({ message: safeApiMessage(err, 'Erro ao listar tarefas') });
    }
  });

  // POST /api/v1/users/:userId/tasks — criar tarefa
  app.post('/api/v1/users/:userId/tasks', auth.requireAuth, requireUuidParams(['userId']), async (req, res) => {
    if (!db.isConfigured() || !db.isConnected()) return res.status(503).json({ message: 'Banco indisponível' });
    const creatorId = req.userId;
    const assigneeId = req.params.userId;
    const { title, description, priority = 'medium', due_date } = req.body || {};
    const titleVal = sanitizeUsername(title);
    if (!titleVal || titleVal.length < 1 || titleVal.length > 200) {
      return res.status(400).json({ message: 'Título é obrigatório (1-200 caracteres)' });
    }
    const priorityVal = ['low', 'medium', 'high', 'urgent'].includes(priority) ? priority : 'medium';
    const descVal = description != null ? String(description).trim() : null;
    let dueDateVal = null;
    if (due_date) {
      const d = new Date(due_date);
      if (!isNaN(d.getTime())) dueDateVal = d.toISOString();
    }
    try {
      const r = await db.query(
        `INSERT INTO tasks (creator_id, assignee_id, title, description, priority, due_date)
         ...
         RETURNING *`,
        [creatorId, assigneeId, titleVal, descVal, priorityVal, dueDateVal]
      );
      const row = r.rows[0];
      return res.status(201).json({
        id: String(row.id),
      ...
      logger.error('POST /api/v1/users/:userId/tasks', err);
      return res.status(500).json({ message: safeApiMessage(err, 'Erro ao criar tarefa') });
    }
  });

  // PATCH /api/v1/tasks/:taskId — atualizar tarefa
  app.patch('/api/v1/tasks/:taskId', auth.requireAuth, requireUuidParams(['taskId']), async (req, res) => {
    if (!db.isConfigured() || !db.isConnected()) return res.status(503).json({ message: 'Banco indisponível' });
    const userId = req.userId;
    ...
    try {
      const existing = await db.query(
        `SELECT * FROM tasks WHERE id = $1::uuid AND (creator_id = $2::uuid OR assignee_id = $2::uuid)`,
        [taskId, userId]
      );
      if (!existing.rows[0]) return res.status(404).json({ message: 'Tarefa não encontrada' });
      const updates = [];
      const params = [];
      let paramIdx = 1;
      if (title !== undefined) {
      ...
      params.push(taskId);
      const sql = `UPDATE tasks SET ${updates.join(', ')} WHERE id = $${paramIdx}::uuid RETURNING *`;
      const r = await db.query(sql, params);
      const row = r.rows[0];
      return res.status(200).json({
        id: String(row.id),
      ...
      logger.error('PATCH /api/v1/tasks/:taskId', err);
      return res.status(500).json({ message: safeApiMessage(err, 'Erro ao atualizar tarefa') });
    }
  });

  // DELETE /api/v1/tasks/:taskId — deletar tarefa
  app.delete('/api/v1/tasks/:taskId', auth.requireAuth, requireUuidParams(['taskId']), async (req, res) => {
    if (!db.isConfigured() || !db.isConnected()) return res.status(503).json({ message: 'Banco indisponível' });
    const userId = req.userId;
    const taskId = req.params.taskId;
    try {
      ...
      if (!existing.rows[0]) return res.status(404).json({ message: 'Tarefa não encontrada' });
      await db.query(`DELETE FROM tasks WHERE id = $1::uuid`, [taskId]);
      return res.status(204).end();
    } catch (err) {
      logger.error('DELETE /api/v1/tasks/:taskId', err);
      return res.status(500).json({ message: safeApiMessage(err, 'Erro ao deletar tarefa') });
    }
  });
