...
use liberty_proto::ServerMessage;
...
pub async fn create_message(
    ...
    let msg = Message {
        ...
        mention_roles: vec![],
        attachments: vec![],
        embeds: req.embeds.clone().unwrap_or_default(),
        reactions: vec![],
        pinned: false,
        created_at: now,
    };
    if let Ok(json) = serde_json::to_string(&ServerMessage::MessageCreated { message: msg.clone() }) {
        state.broadcast_for_channel(channel_id, &json).await;
    }
...
pub async fn update_message(
    ...
    if let Ok(json) = serde_json::to_string(&ServerMessage::MessageUpdated {
        channel_id,
        message_id,
        content: req.content.clone(),
...
pub async fn delete_message(
    ...
    if let Ok(json) = serde_json::to_string(&ServerMessage::MessageDeleted {
        channel_id,
        message_id,
    }) {
        state.broadcast_for_channel(channel_id, &json).await;
...
pub async fn list_pins(
    ...
    let rows = sqlx::query_as::<_, (String, String, String, String, Option<String>, i32, i32, i32, String)>(
        r#"
        SELECT id, channel_id, author_id, content, edited_at, tts, mention_everyone, pinned, created_at
        FROM messages WHERE channel_id = ? AND pinned = 1 ORDER BY created_at DESC
        "#
    )
    .bind(channel_id.to_string())
    .fetch_all(&state.db)
