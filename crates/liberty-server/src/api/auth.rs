...
use liberty_core::{User, UserStatus};
...
/// Verify email request (optional token from verification email)
#[derive(Debug, Deserialize)]
...
pub async fn logout(
    ...
    if let Some(auth_header) = request.headers().get("Authorization").and_then(|h| h.to_str().ok()) {
        let token = if auth_header.starts_with("Bearer ") { &auth_header[7..] } else { auth_header };
        if let Ok(claims) = state.jwt.validate_access_token(token) {
            let _ = sqlx::query("INSERT OR IGNORE INTO token_blacklist (token_jti, expires_at) VALUES (?, ?)")
                .bind(&claims.jti)
    ...
    state.sessions.retain(|_, v| *v != user_id);
...
pub async fn verify(
    ...
    let token = match &req.token {
        ...
        _ => {
            return Err((
                ...
                Json(ErrorResponse {
                    code: 400,
                    message: "Verification token required".to_string(),
                }),
            ));
        }
    };
    ...
    .map_err(|e| (
        StatusCode::INTERNAL_SERVER_ERROR,
        Json(ErrorResponse { code: 500, message: e.to_string() }),
    ))?;
    ...
    if let Ok(exp) = chrono::DateTime::parse_from_rfc3339(&expires_at) {
        if exp.with_timezone(&Utc) < Utc::now() {
            let _ = sqlx::query("DELETE FROM verification_tokens WHERE token = ?")
                .bind(&token)
                .execute(&state.db)
                .await;
            return Err((
