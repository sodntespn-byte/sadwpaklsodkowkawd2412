...
fn row_to_server(row: ServerRow) -> Server {
    Server {
        ...
        updated_at: chrono::DateTime::parse_from_rfc3339(&row.updated_at)
            .map(|d| d.with_timezone(&Utc))
            .unwrap_or_else(|_| Utc::now()),
        max_members: row.max_members.map(|v| v as u64),
...
pub async fn get_server(
    ...
    // Get members with their roles from member_roles
    ...
    for r in member_rows {
        let uid = Uuid::parse_str(&r.1).unwrap();
        let roles: Vec<Uuid> = sqlx::query_scalar::<_, String>(
...
pub async fn delete_server(
    ...
    // Cascade delete: delete in order to satisfy foreign keys
    let sid = id.to_string();
    sqlx::query("DELETE FROM reactions WHERE message_id IN (SELECT id FROM messages WHERE channel_id IN (SELECT id FROM channels WHERE server_id = ?))")
        .bind(&sid)
        .execute(&state.db)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(super::auth::ErrorResponse { code: 500, message: e.to_string() })))?;
    sqlx::query("DELETE FROM message_mentions WHERE message_id IN (SELECT id FROM messages WHERE channel_id IN (SELECT id FROM channels WHERE server_id = ?))")
        .bind(&sid)
        .execute(&state.db)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(super::auth::ErrorResponse { code: 500, message: e.to_string() })))?;
    sqlx::query("DELETE FROM attachments WHERE message_id IN (SELECT id FROM messages WHERE channel_id IN (SELECT id FROM channels WHERE server_id = ?))")
        .bind(&sid)
        .execute(&state.db)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(super::auth::ErrorResponse { code: 500, message: e.to_string() })))?;
    ...
    sqlx::query("DELETE FROM channels WHERE server_id = ?")
        ...
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(super::auth::ErrorResponse { code: 500, message: e.to_string() })))?;
    sqlx::query("DELETE FROM member_roles WHERE server_id = ?")
        .bind(&sid)
        .execute(&state.db)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(super::auth::ErrorResponse { code: 500, message: e.to_string() })))?;
