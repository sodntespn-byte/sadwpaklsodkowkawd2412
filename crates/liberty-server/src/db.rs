...
pub async fn init_database() -> Result<SqlitePool> {
    ...
    // Enable foreign key support (required for cascade / integrity)
...
async fn create_tables(pool: &SqlitePool) -> Result<()> {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_channels_server ON channels(server_id)")
        .execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_relationships_user ON relationships(user_id)")
        .execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_relationships_target ON relationships(target_id)")
        .execute(pool).await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_dm_recipients_user ON dm_recipients(user_id)")
        .execute(pool).await?;
