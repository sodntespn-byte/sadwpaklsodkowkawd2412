...
- **Rate limiting** — Limite geral por IP (por minuto) e limite mais restritivo em `/api/v1/auth` (anti força bruta).
- **Validação de entradas** — Joi em registo, login, refresh e corpo de mensagens para reduzir risco de injeção e XSS.
